...
class LevelGen(MARoomGridLevel):
    """
    ...
    def gen_mission(self):
        ...
        for agent in self.agents:
            while True:
                pos = self.place_agent(agent)
                start_room = self.room_from_pos(*pos)
        ...
        if not self.unblocking:
            self.check_objs_reachable(self.agents)
    ...
    def rand_obj(self, types=OBJ_TYPES, colors=COLOR_NAMES, max_tries=100):
        ...
        while True:
            ...
            if not self.implicit_unlock and isinstance(self.locked_room, MARoom):
